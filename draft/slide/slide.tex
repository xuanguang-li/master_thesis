...
\begin{frame}{Investment demand shock and its origin}
	...
	\textbf{Idiosyncratic (news) technology shock} $\varepsilon$ increases threshold:
...
\begin{frame}{Why idiosyncratic shocks do not wash out: a GE multiplier}
	...
	\begin{tikzpicture}[
		...
		\node[box, shockc, draw=shockc] (shock) at (0.8,1.9) {idiosyncratic shock $\varepsilon$\\ on a few firms};
		\node[box, invest, draw=invest] (inv) at (5.3,1.9) {extra fraction $\nu$ crosses\\ threshold $\Rightarrow$ lumpy spikes};
		\node[box, accent, draw=accent] (Y) at (8,0) {future output $Y_{t+1}\uparrow$};
		\node[box] (thr) at (2.6,0) {\emph{every} firm's threshold\\ rises ($k^{*}\propto Y_{t+1}$)};
		\draw[arr, shockc] (shock) -- (inv);
		\draw[arr] (inv) -- (Y);
		\draw[arr] (Y) -- (thr);
		\draw[arr] (thr) -- (inv);
		\node[font=\scriptsize\itshape, gray] at (5.6,0.8) {GE multiplier};
	\end{tikzpicture}
...
\begin{frame}{The estimated DSGE model}
	...
	\begin{itemize}
		\item \textbf{Nominal rigidities:} Calvo price setting.
		\item \textbf{Other frictions:} 
		\begin{itemize}
			\item Investment adjustment cost and endogenous capital utilization.
			\item reduced-form sticky real wage:
			\(
...
\begin{frame}{Baseline result: variance decomposition}
	...
	\textcolor{invest}{\textbf{ID shock}} alone explains $\approx\!\mathbf{20\%}$ of output, $\approx\!\mathbf{27\%}$ of hours worked and $\approx\!\mathbf{25\%}$ of investment FEVD.
	...
	\hline
	Observable & $\varepsilon_g$ & \textcolor{accent}{$\varepsilon_s$} & $\varepsilon_R$ & $\varepsilon_a$ & \textcolor{invest}{$\varepsilon_d$} & $\mathrm{meas}$ \\ 
	\hline
	Output (level) & 2.48 & 31.77 & 3.40 & 46.81 & \textbf{19.97} & 1.38 \\ 
	Inflation & 0.32 & 12.21 & 26.73 & 47.43 & 11.13 & 1.62 \\ 
	Interest rate & 0.42 & 18.55 & 14.76 & 48.89 & 14.30 & 4.52 \\ 
	Hours (level) & 3.73 & 49.06 & 1.70 & 14.27 & \textbf{26.52} & 8.24 \\ 
	Investment (level) & 0.45 & 55.80 & 0.26 & 19.66 & \textbf{24.67} & 1.37 \\ 
	Gov. spending (level) & 95.93 & 0.07 & 0.01 & 0.09 & 0.07 & 3.79 \\ 
	\hline
	\end{tabular}
	...
	{\footnotesize
	Frequency-domain FEVD (\%), business-cycle band \textbf{6--32} quarters. The decomposition is obtained using the spectrum of the DSGE model and an inverse first difference filter for output, investment and government spending to reconstruct the levels.}
...
\begin{frame}{Robustness: Smets--Wouters-style model}
	...
	\begin{itemize}
		\item Add habit formation, Erceg nominal wage rigidity, preference / price- / wage-markup shocks, and consumption and wage growth as observables.
...
\begin{frame}{Takeaway}
	...
	\begin{itemize}
		...
		\item \textbf{Baseline:} 
		\begin{itemize}
			\item The investment demand shock alone explains $\approx 29\%$ of output, $\approx 27\%$ hours worked and $\approx 25\%$ of investment FEVD, which is a competitive driver.
			\item The investment demand shock generates an inflationary expansion with comovement (vs.\ deflationary IST).
		\end{itemize}
