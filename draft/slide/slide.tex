...
\begin{frame}{A toy model of idiosyncratic shocks and lumpy investment}
	...
	\textbf{Intuition:} (i) firms decide their investment according to the aggregate output because aggregate ouput influence the demand for their own product; (ii) if some firms increase their investment, future output will increase as well; (iii) thus other firms will response to invest more.
	\vspace{1em}
...
\begin{frame}{Idiosyncratic shock $\to$ investing fraction}
	% --- top region: text (left) + figure pinned NE (right) ---
	\vspace{1em}
	\begin{columns}[T,onlytextwidth]
...
\begin{frame}{Investing fraction $\to$ aggregate output $\to$ investing fraction}
	% --- top region: text (left) + figure pinned NE (right) ---
	\begin{columns}[T,onlytextwidth]
		\column{0.8\textwidth}
...
\begin{frame}{Conclusion}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item \textbf{Takeaway:} structural estimation gives insight into investment-side shocks, but the model specification must be chosen carefully.
	\end{itemize}
\end{frame}
\end{document}
