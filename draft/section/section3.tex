...
\begin{equation}
    \ln \mu_{d, t} = (1 - \rho_d) \ln \mu_d + \rho_d \ln \mu_{d, t-1} + \varepsilon_{d, t},\,\, \varepsilon_{d, t} \sim \mathcal{N}(0, \sigma_d^2)
\end{equation}
...
\begin{equation}\label{eq:new_hh_optimal}
    \begin{aligned}
        \max\limits_{c, b, k, l, u} \, &\mathbb{E}_t \sum_{k=0}^{\infty} \beta^k \left(\frac{(c_{t+k} / \gamma^{t+k})^{1-\sigma} - 1}{1 - \sigma} - \chi\frac{\ell_{t+k}^{\sigma_\ell + 1}(j)}{\sigma_\ell + 1}\right) \\
...
\begin{figure}[htbp]
    ...
    \input{figures/plot_model_relation.tex}
    }
