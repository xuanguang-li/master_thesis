...
\begin{equation}
    \mathbb{E}_t \left[ \sum_{s=0}^{\infty} \beta^s \mu_{b, t+s} \left( \ln(C_{t+s} - h C_{t+s-1}) - \chi \frac{L_{t+s}(j)^{1+\sigma_\ell}}{1+\sigma_\ell} \right) \right],
\end{equation}
...
It is worth mentioning that the consumption part in the utility function becomes logarithmic instead of the more general CRRA form. This is because with habit formation and a separable utility function between $C_t$ and $L_t$, a general CRRA utility function does not have a balanced growth path unless it reduces to a logarithmic form.
...
I revisit the consumption impact in Figure~\ref{fig:sw:con}. Compared with the similar exercise in baseline result (Figure~\ref{fig:inv_demand_c1_kappa_psipi_heatmap}), I choose the parameters, the inverse Frisch elasticity $\sigma_\ell$ and Taylor-rule output-gap coefficient $\psi_y$, to be varied. This is because the Phillips curve, as well as the wage Phillips curve, becomes quite flat. Thus, the slopes themselves have already approached the \cite{nireiOriginsPropagationAnimal} calibration, and further, the inflation feedback in the Taylor rule becomes less important because inflation hardly moves. However, the logic stays the same---the consumption impact is decided by the path of the real interest rate, so the nominal interest rate helps. If the nominal interest rate is low enough, the real interest rate can also be very low, and thus the intertemporal substitution effect will be tempered. Smaller $\psi_y$ explicitly does this job by lowering output gap feedback in Taylor's rule. Smaller $\sigma_\ell$ works through the output gap by suppressing the real wage---because the nominal wage is very sticky, the wage gap is smaller if the real wage barely moves, and thus the output gap becomes smaller.
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/inv_demand_c1_psiy_sigmal_heatmap.pdf}
    \caption{Impact consumption response \(\hat c_1\) to a unit investment-demand shock as a function of the inverse Frisch elasticity \(\sigma_\ell\) and the Taylor-rule output-gap coefficient \(\psi_y\). All other parameters are held at the posterior mean. }
    \label{fig:sw:con}
\end{figure}
...
\end{equation}
and thus there will be a weak identification problem with the previous data set.
