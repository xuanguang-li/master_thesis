...
By adding such shock, wage is less tied with $MRS_t$, and thus it has more freedom to fit with the data. In \ref{sec:robustness}, the estimation based on the \cite{smetsShocksFrictionsUS2007} model shows a much better fit for the variance of hours worked, and the FEVD analysis in Table~\ref{tab:vd:sw-type-no-rel} also shows the strong influence of the wage markup shock on hours worked and wage growth.
